...
<form name="form2" method="post" action="ListDolog.php" onsubmit="return confirm('确认要删除?');">
  ...
  ?>
    <tr bgcolor="#e5e8eb" id=log<?=$r[logid]?>> 
      <td> 
      ...
      <td height="25"> 
        <div align="center">[<a href="ListDolog.php?enews=DelDoLog&logid=<?=$r[logid]?><?=$ecms_hashur['href']?>" onclick="return confirm('确认要删除此日志?');">删除</a> 
          <input name="logid[]" type="checkbox" id="logid[]" value="<?=$r[logid]?>" onclick="if(this.checked){log<?=$r[logid]?>.style.backgroundColor='#cccccc';}else{log<?=$r[logid]?>.style.backgroundColor='#e5e8eb';}">
          ]</div></td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
